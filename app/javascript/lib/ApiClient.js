...
const apiClient = {
  ...
  createCard: function(body, callback) {
    return axios.post(routes.CREATE_CARD_URL, body)
      .then(unwrapData)
      .then(callback)
      .catch(logError);
  },
  fetchCard: function(cardId, callback) {
    return axios.get(routes.SHOW_CARD_URL(cardId), callback)
      .then(unwrapData)
      .then(callback)
      .catch(logError);
  },
  updateCard: function(cardId, updatedCard, callback) {
    return axios.put(routes.UPDATE_CARD_URL(cardId), { card: updatedCard })
      .then(unwrapData)
      .then(callback)
      .catch(logError);
  }
